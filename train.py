...
def main(args=None):
    ...
    for epoch_num in range(0, parser.epochs):
        ...
        for iter_num, data in enumerate(dataloader_train):
            ...
            classification_loss, regression_loss, repgt_loss, repbox_loss = retinanet([data['img'].cuda().float(), data['annot'], data['ignore']])
            ...
            print('Epoch: {} | Iteration: {} | Classification loss: {:1.5f} | Regression loss: {:1.5f} | RepGT loss {:1.5f} | RepBox loss {:1.5f} | Running loss: {:1.5f}'.format(epoch_num, iter_num, float(classification_loss), float(regression_loss), float(repgt_loss), float(repbox_loss), np.mean(loss_hist)))
