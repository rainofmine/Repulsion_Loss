...
class CSVDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        annot, igno = self.load_annotations(idx)
        sample = {'img': img, 'annot': annot, 'ignore': igno, 'scale': 1}
        if self.transform:
...
def collater(data):
    ...
    return {'img': padded_imgs, 'annot': annot_padded, 'ignore': ig_padded, 'scale': scales}
...
class Resizer(object):
    ...
    def __call__(self, sample, min_side=800, max_side=1400):
        ...
        image, annots, igno, scale = sample['img'], sample['annot'], sample['ignore'], sample['scale']
        ...
        return {'img': new_image, 'annot': annots, 'ignore': igno, 'scale': scale}
...
class Augmenter(object):
    ...
    def __call__(self, sample, flip_x=0.5):
        ...
        if np.random.rand() < flip_x:
            image, annots, igno, scales = sample['img'], sample['annot'], sample['ignore'], sample['scale']
            image = image[:, ::-1, :]
            ...
            sample = {'img': image, 'annot': annots, 'ignore': igno, 'scale': scales}
...
class Random_crop(object):
    ...
    def __call__(self, sample):
        ...
        if not annots.shape[0]:
            return {'img': image, 'annot': annots, 'ignore': igno, 'scale': scales}
        if random.choice([0, 1]):
            ...
            while True:
                ...
                new_igno = np.zeros((0, 5))
                for i in range(igno.shape[0]):
                    x1 = max(igno[i, 0] - cols_zero, 0)
                    y1 = max(igno[i, 1] - rows_zero, 0)
                    x2 = min(igno[i, 2] - cols_zero, crop_cols)
                    y2 = min(igno[i, 3] - rows_zero, crop_rows)
                    label = igno[i, 4]
                    if x1 + 10 < x2 and y1 + 5 < y2:
                        x1 /= crop_ratio
                        y1 /= crop_ratio
                        x2 /= crop_ratio
                        y2 /= crop_ratio
                        new_igno = np.append(new_igno, np.array([[x1, y1, x2, y2, label]]), axis=0)
                '''
                ...
                new_igno = np.zeros((0, 5))
                for i in range(igno.shape[0]):
                    x1 = max(cols_zero, igno[i, 0])
                    y1 = max(rows_zero, igno[i, 1])
                    x2 = min(cols_zero + crop_cols, igno[i, 2])
                    y2 = min(rows_zero + crop_rows, igno[i, 3])
                    label = igno[i, 4]
                    if x1+10 < x2 and y1+5 < y2:
                        new_igno = np.append(new_igno, np.array([[x1, y1, x2, y2,label]]), axis=0)

                return {'img': new_image, 'annot': new_annots, 'ignore': new_igno, 'scale': scales}
...
class Normalizer(object):
    ...
    def __call__(self, sample):
        ...
        sample = {'img': torch.from_numpy(image), 'annot': torch.from_numpy(annots), 'ignore':torch.from_numpy(igno), 'scale': scales}
        return sample
